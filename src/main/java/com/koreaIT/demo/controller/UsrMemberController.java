...
public class UsrMemberController {
	...
	public ResultData<Member> doJoin(HttpSession httpSession, String loginId, String loginPw, String name, String nickname, String cellphoneNum, String email) {
		...
		return ResultData.from(doJoinRd.getResultCode(), doJoinRd.getMsg(), "memberId",memberService.getMemberById((int)doJoinRd.getData1()));
	...
	public ResultData doLogin(HttpSession httpSession, String loginId, String loginPw) {
		...
		if (httpSession.getAttribute("loginedMemberId") != null) {
			return ResultData.from("F-B", "로그아웃 후 이용해주세요.");
		}
		...
		if (member == null) {
			return ResultData.from("F-3", Util.f("%s은(는) 존재하지 않는 아이디 입니다.", loginId));
		}
		...
		return ResultData.from("S-1", Util.f("%s회원님 환영합니다.", member.getNickname()));
		}
